...
function preload() {
  ...
  spriteData=loadJSON("assets/boat/boat.json")
   ...
   brokenImage=loadImage("assets/boat/brokenBoat.png")
   aguaData=loadJSON("assets/waterSplash/waterSplash.json")
   aguaImage=loadImage("assets/waterSplash/waterSplash.png")
...
var aguaframes=aguaData.frames
for(var i=0;i<aguaframes.length;i++){
var pos=aguaframes[i].position 
var img =aguaImage.get(pos.x,pos.y,pos.w,pos.h)
...
function showBoats(){
  ...
  else{barco=new Barco(width,height-60,170,170,-60, barcoAnimation)
    barcos.push(barco)}
}
